...
from make_random_point_mask import make_random_point_mask_for_train_for_label_Non_directional,\
                                                            make_random_point_mask_directional
#
def my_make_mask(img):
    mask=make_random_point_mask_directional(img)
    return mask
#####################################
...
class InpaintDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        grayscale = data_enchanement(grayscale)
        ##################################################
...
class ValidationSet_with_Known_Mask(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # mask
        mask_up_path = os.path.join(self.opt.maskroot, imgname.replace('.jpg',''))
        img = torch.from_numpy(img.astype(np.float32) / 255.0).unsqueeze(0).contiguous()
        return img, mask_up_path, imgname
        #########################
