...
ALLMASKTYPES = ['single_bbox', 'bbox', 'free_form', 'my_mask']
...
def data_enchanement(img, opt):
    ...
    if opt.resize_or_crop == 'resize_and_crop':
        random_value=random.random()
        if random_value < 0.5 :
            img=cv2.resize(img, opt.loadSize)
            x_offset=random.randint(0,opt.loadSize[0]-opt.fineSize[0])
            y_offset=random.randint(0,opt.loadSize[1]-opt.fineSize[1])
            x_l=x_offset+opt.fineSize[0]
            y_l=y_offset+opt.fineSize[0]
            img=img[x_offset:x_l,y_offset:y_l]
    return img
...
class DomainTransferDataset(Dataset):
    def __init__(self, opt):
        ...
        self.opt = opt
        self.imglist_A = get_files(opt.baseroot_A)
        self.imglist_B = get_files(opt.baseroot_B)
...
class InpaintDataset(Dataset):
    def __init__(self, opt):
        ...
        self.opt = opt
        self.imglist = get_jpgs(opt.img_file)
    ...
    def __getitem__(self, index):
        ...
        imgpath = os.path.join(self.opt.img_file, imgname)                  # path of one image
        ###############################################################################################
        ...
        grayscale = Image.open(imgpath)
        grayscale = np.array(grayscale)
        grayscale = data_enchanement(grayscale, self.opt)
        ret, grayscale = cv2.threshold(grayscale,127,255,cv2.THRESH_BINARY)
        ##################################################
        ...
        ##################################################################################################
        if self.opt.mask_type == 'my_mask':
            mask = my_make_mask(grayscale)
            #####################
            mask=mask.reshape((1, ) + mask.shape).astype(np.float32)
        # mask = torch.from_numpy(mask).contiguous()
        mask = torch.from_numpy(mask / 255.0).contiguous()
        #######################################
...
class ValidationSet_with_Known_Mask(Dataset):
    def __init__(self, opt):
        self.opt = opt
        self.namelist = get_jpgs(opt.img_file)
