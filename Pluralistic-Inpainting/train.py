...
if __name__ == '__main__':
    ...
    dataset = torch.utils.data.DataLoader(dataset, batch_size = opt.batchSize, shuffle = True, num_workers = opt.nThreads, pin_memory = True)
    #############################
    ...
    while(keep_training):
        ...
        ###################
        for i, (img, mask, img_name) in enumerate(dataset):
            mask = 1 - mask  
            data={'img': img, 'mask': mask, 'img_path': img_name }
            ###################
